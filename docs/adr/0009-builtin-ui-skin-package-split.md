...
- The historical `assets/ui-skin/registry.json` selected the default directory
  ...
  staging uses the locally supplied manager snapshot without a source lock.
...
structurally validated snapshot and locally resolved assets; it does not read
the old registry or source directory.
