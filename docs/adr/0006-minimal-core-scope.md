...
日期：2026-09-14（2026-09-17 完成 Task 3.1 最终态复核）
状态：Accepted（当前有效裁决 = v3 严格边界 + v4 源码删除 + v5 接口面收敛）

## 当前有效状态索引

本文按时间保留 v2 至 v5 的决策过程，但实现只能以以下最终裁决为准：

- **v2 是历史提案，不是当前运行时契约。** v2 所述 recovery 页面、
  `rc.*` / `rescue.*` / `guard.*` 降级桩和“剥离代码原位保留”均已被后续
  裁决覆盖。
- **v3 确立严格最简边界；v4 要求剥离源码；v5 收敛公开接口。** 当前仓库已
  删除 recovery 页面/preload、recovery/rescue 源码与公开 RPC 桩；退役方法
  返回 JSON-RPC `-32601`，退役壳页返回 HTTP 404。
- **当前恢复方式是 Git 历史 + 后续包接入。** 未来能力接回时，从历史恢复
  实现与测试、补包清单和装配入口；本体不保留空页面、占位 RPC 或无消费者
  模块作为“插口”。
