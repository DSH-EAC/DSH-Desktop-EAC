...
exports.resolveTagCommit = resolveTagCommit;
...
// 共享出口 IP；缓存冷时矩阵 4 个 job 同时打到这一行会互相抢配额，输的那个收到
// HTTP 403 直接判失败（随机哪个 job 挂）。因此：
//   1. 钉版 tag 直接把 commit 写进 KERNEL_TAG_COMMITS，零网络请求；
//   2. 其余 tag（`npm run fetch-kernel -- <tag>` 升级路径）用 `git ls-remote`
//      解析 —— 脚本本就依赖 git，且该协议不受 API 配额限制。
 ...
 * 不再触达匿名 GitHub API 配额。升级内核（改 DEFAULT_TAG）时必须同步补一条
 * ——kernel-pin-consistency.test.ts 会强制这条记录存在且格式合法。
 */
const KERNEL_TAG_COMMITS = {
...
function capture(command, args) {
    ...
    if (result.status !== 0 || result.error !== undefined) {
        // 带出退出码与 stderr：issue #393 定位时全靠反推别的 job 才看出是 HTTP 403，
        // `capture()` 把 stderr 吞掉是那次排查最大的障碍。
        const detail = [
            `退出码 ${String(result.status ?? '(未启动)')}`,
            result.error === undefined ? '' : `错误 ${result.error.message}`,
            (result.stderr ?? '').trim(),
        ].filter(Boolean).join('；');
...
/**
 * 解析 tag 指向的 commit SHA（40 位小写十六进制）。
 *
 * 优先查钉版表（零网络）；未命中才回落 `git ls-remote`。
 * 注意**不要**改回 GitHub REST API：匿名配额按出口 IP 计，CI 矩阵并发时必然
 * 互相抢配额（issue #393）。`ls-remote` 走 git 协议，不受该配额限制。
 *
 * @param pins   可注入的钉版表（测试用；默认 KERNEL_TAG_COMMITS）
 * @param remote 可注入的仓库地址（测试指向本地裸库即可离线覆盖两条分支）
 */
function resolveTagCommit(tag, pins = KERNEL_TAG_COMMITS, remote = `https://github.com/${REPO}.git`) {
    const pinned = pins[tag];
    if (pinned !== undefined) {
        if (!COMMIT_SHA.test(pinned)) {
            throw new Error(`KERNEL_TAG_COMMITS['${tag}'] 不是 40 位小写十六进制: ${pinned}`);
        }
        console.log(`fetch-kernel: commit ${pinned}（钉版表，未请求网络）`);
        return pinned;
    }
    // 附注 tag 用 `^{}` 拿 peel 后的 commit；轻量 tag（当前钉版即是）`^{}` 返回空，
    // 回落裸 ref。两种 ref 各试一次，前者空即自动落到后者。
    for (const ref of [`refs/tags/${tag}^{}`, `refs/tags/${tag}`]) {
        const line = capture('git', ['ls-remote', remote, ref]);
        const sha = line.split(/\s+/)[0];
        if (sha !== undefined && COMMIT_SHA.test(sha)) {
            console.log(`fetch-kernel: commit ${sha}（git ls-remote ${ref}）`);
            return sha;
        }
    }
    throw new Error(`git ls-remote 未能解析 ${tag} 指向的 commit（${remote}；请确认 tag 存在）`);
}
...
function main() {
    ...
    const commitSha = resolveTagCommit(tag);
    const baseEnv = {
...
// 仅命令行直接执行时跑 main()：测试 require 本模块只为拿 resolveTagCommit，
// 不应触发真实的下载/构建链（同 patch-deps.ts 的守卫写法）。
...
module.exports = { capture, resolveTagCommit, KERNEL_TAG_COMMITS, DEFAULT_TAG };
