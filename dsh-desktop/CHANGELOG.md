...
新增内置全局技能 `research-first`（`assets/skills`，随客户端分发）：把
「先检索再作答」固化成技能 —— 遇到不确定的问题按「本机记忆 → 联网搜索 →
GitHub / npm 找现成工具」三路取证，含停止条件、来源可信度判断、第三方
插件依赖评估清单与反模式；首次启动自动同步到 `~/.dsh/skills/`，用户自建
同名技能不被覆盖 →
