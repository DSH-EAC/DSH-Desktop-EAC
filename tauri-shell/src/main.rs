...
fn died_page(log_path: &str, code: &str) -> String {
    ...
    format!(
        "<!doctype html><html lang={0}><meta charset=utf-8><title>{1}</title>{SHELL_SKIN_LINKS}\
         ...
         <button onclick=\"retry()\" style=\"padding:8px 22px;border:1px solid var(--eac-shell-border-strong,rgba(255,255,255,.18));\
         border-radius:9px;background:var(--eac-shell-accent-soft-bg-strong,rgba(91,140,255,.15));color:var(--eac-shell-text-primary,#dfe6ff);font-size:13px;cursor:pointer\">{6}</button>\
         <button onclick=\"safeMode()\" style=\"padding:8px 22px;border:1px solid var(--eac-shell-safe-mode-border,rgba(255,200,120,.25));\
         border-radius:9px;background:var(--eac-shell-safe-mode-bg,rgba(255,180,80,.10));color:var(--eac-shell-safe-mode-text,#ffd9a3);font-size:13px;cursor:pointer\">{7}</button>\
         </div>\
         </div>\
...
fn wizard_page() -> String {
    ...
    let html = std::fs::read_to_string(&file).unwrap_or_else(|_| {
        format!(
            "<!doctype html><meta charset=utf-8><title>{0}</title>{SHELL_SKIN_LINKS}<body style=\"background:var(--eac-shell-bg-base,#0b1220);color:var(--eac-shell-text-primary,#dfe6ff);font-family:sans-serif;display:grid;place-items:center;height:100vh\">{1} (assets/onboarding.html)</body>",
            ui_text("向导", "Wizard"),
